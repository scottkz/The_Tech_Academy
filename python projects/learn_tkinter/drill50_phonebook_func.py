...
def addToList(self):
    ...
    if (
        ...
        with conn:
            ...
            if (
                chkName == 0
            ):  # if this is 0 then there is no existence of the fullname and we can add new data
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        """DELETE FROM tbl_phonebook WHERE col_fullname = '{}'""".format(
                            var_select
                        )
                    )
                onDeleted(
                    self
                )  # call the function to clear all of the textboxes and the selected index of listbox
